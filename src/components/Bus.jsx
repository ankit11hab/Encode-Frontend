...
function Bus({ data }) {
  ...
  const Arrival_container = styled.div`
    // border: 1px solid black;
  `;
  const Arrival_time = styled.h3``;
  const Arrival_address = styled.p``;
    ...
    <Main_container>
      <Container>
        <Name_container>
          ...
          <Span>
            <Icon src='https://news.northeastern.edu/wp-content/uploads/2020/05/mask_icon.png'></Icon>
          </Span>
          <Span>
        ...
        <Arrival_container>
          <Arrival_time>{data.drop_time} Hours</Arrival_time>
          <Arrival_address>{data.drop_address}</Arrival_address>
        </Arrival_container>
        <Rating_container>
        ...
        <Fare_container>
          Starts from INR
          <span>
            <Ticket_price>₹ {data.price}</Ticket_price>
          </span>
        </Fare_container>
        <Seat_container>
      ...
      <ViewSeat_btn>
        ...
        <Toggle_button onClick={() => setToggle(!toggle)}>
          View Seats
        </Toggle_button>
      </ViewSeat_btn>
      </Link>
